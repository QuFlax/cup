...
static Node zero = { N_NUMBER, nullptr, 0 };
static Node root;

static std::vector<const char*> global_strings = {};
static std::vector<Node> node_stack;
static std::vector<void*> extra_stack; // або інший тип, залежно від вмісту
...
std::ostream& operator<<(std::ostream& os, const Node n) {
  ...
  case N_CONTROLFLOW:
    ...
    else if (n.as.token == T_WHILE) {
      return os << "{ while ( " << n.nodes.front() << " ) " << n.nodes.back() << " }";
    }
    else {
...
const Node primary(uint8_t mpower) {
  ...
  switch (CurTok) {
  case T_ORB: {
    left = (getNextToken(true) == T_CRB) ? empty : primary();
    expectandnext(T_CRB);
    break;
  }
  case T_IDENTIFIER: {
  ...
  case T_OSB: {
    ...
    if (arr.type == N_OPERATOR && arr.as.token == T_COMMA) {
  ...
  case T_SUB: {
    getNextToken(true);
    left.type = N_OPERATOR;
    left.as.token = CurTok;
    left.nodes.push_back(zero);
    left.nodes.push_back(primary(getMaxPower));
    break;
  }
  default:
...
const Node statement() { // statement
  ...
  case T_AT: { // function
    ...
    Node protoname = { N_VARIABLE, this_scope };
    protoname.as.inumber = token_num;
    expectandnext(T_IDENTIFIER);

    Node proto = { N_CALL, this_scope };
    proto.nodes.push_back(protoname);

    Scope* oldScope = this_scope;
    this_scope = new Scope();
    this_scope->parent = oldScope;

    proto.nodes.push_back(primary());
